...
func (c *CephClient) flattenImage(ctx context.Context, imageName string, progressCb func(msg string, pct int)) error {
        ...
        // Flatten 是同步阻塞操作，在 goroutine 中执行以支持取消
        ...
        case err := <-done:
                return err
        case <-ctx.Done():
                // 注意：librbd 的 Flatten 不能真正中断，这里只是返回取消信号
                // 实际 flatten 会继续在后台完成
                log.Println("Flatten operation cancellation requested (librbd will complete the operation)")
                return ctx.Err()
        }
}

// ImageExists 检查 image 是否存在
func (c *CephClient) ImageExists(imageName string) (bool, error) {
        img, err := rbd.OpenImage(c.ioctx, imageName, rbd.NoSnapshot)
        if err != nil {
                // 判断是否为 not found 错误
                return false, nil
...
// GetImageSize 获取 image 大小
func (c *CephClient) GetImageSize(imageName string) (uint64, error) {
        img, err := rbd.OpenImage(c.ioctx, imageName, rbd.NoSnapshot)
        if err != nil {
                return 0, err
        }
...
func (s *Server) executeCopy(task *CopyTask) {
        ...
        existing, err := dstK8s.CoreV1().PersistentVolumeClaims(task.Request.DstNS).Get(
...
func buildK8sClient(cfg *K8sClusterConfig) (kubernetes.Interface, error) {
        ...
        var restCfg *clientcmd.ClientConfig
        _ = restCfg
...
func testConnection() {
        ...
        kubeconfigPath := "/root/.kube/cluster-a"  // 改成你实际的路径
        ...
        log.Printf("API Server: %s", restConfig.Host)
        log.Printf("Timeout: %v", restConfig.Timeout)
        log.Printf("TLS Insecure: %v", restConfig.TLSClientConfig.Insecure)
        ...
        log.Printf("成功! 版本: %s (耗时: %v)", version.String(), elapsed)
        ...
        pvcs, err := client.CoreV1().PersistentVolumeClaims("mhs-test-mfa").List(ctx, metav1.ListOptions{})
        elapsed = time.Since(start)

        if err != nil {
                log.Printf("列 PVC 失败: %v (耗时: %v)", err, elapsed)
                return
        }
        log.Printf("成功! PVC 数量: %d (耗时: %v)", len(pvcs.Items), elapsed)
